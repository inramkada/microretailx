...
function openConsentPanel() {
  if (!consentModalBackdrop) return;
...
function saveFromPanel() {
  ...
  const analytics = !!(toggleAnalytics && toggleAnalytics.classList.contains("on"));
  const marketing = !!(toggleMarketing && toggleMarketing.classList.contains("on"));
...
function toggleSwitch(el) { if (el) setToggle(el, !el.classList.contains("on")); }
function handleSwitchKey(el, e) {
  if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleSwitch(el); }
}

/* =========================
   LEGAL GATE (blocking ALL /legal/*)  ✅ PATCHED
...
function initLegalGate() {
  // Capture phase so we win against other handlers
...
/* =========================
   Canvas / particles (unchanged below)
   ========================= */
